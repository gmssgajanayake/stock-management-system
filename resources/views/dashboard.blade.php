    ...
    <div x-data="{ open: false }" class="flex h-full w-full relative"> <!-- Mobile Overlay -->
        ...
        <div :class="open ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="lg:hidden flex justify-end p-4"> <button @click="open = false"
                    class="text-white text-xl">✕</button> </div> {{-- seperate here adding MAIN lable --}} <div class="mt-8 flex flex-col">
            ...
            @role('admin')
                <div class="mt-6 flex flex-col">
                    <span class="px-4 text-gray-400 uppercase text-xs font-semibold">Admin</span>

                    <a href="{{ route('users.index') }}" class="w-full hover:bg-slate-500 p-4">Users</a>
                    <a href="{{ route('roles.index') }}" class="w-full hover:bg-slate-500 p-4">Roles</a>
                    <a href="{{ route('permissions.index') }}" class="w-full hover:bg-slate-500 p-4">Permissions</a>
                </div>
            @endrole
